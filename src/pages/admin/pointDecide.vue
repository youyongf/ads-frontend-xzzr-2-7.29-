<template>
  <div>
    <a-row class="query-form">
      <a-button  v-if="isAdmin" @click="addPointDecide" type="primary" icon="plus" style="float: left;">{{$tcache('common.add')}}</a-button>
      ...
      <a-button @click="getAlldecisionPoint" class="btn"><span class="fontSty">{{$tcache('pointDecide.getAlldecisionPoint')}}</span></a-button>
    </a-row>
    ...
    <a-modal
        ...
        @cancel="handleCancel"
    >
      <!-- <Pdetail :data="record" :type="type" :cacheStations="cacheStations"/> -->
...
export default {
  ...
  methods:{
    ...
    getAlldecisionPoint(){
      this.axios.get(`voluntarily/decideAllInstall`).then(res=>{
        if(res.data.success){
          this.isSave=true
          this.queryData()
    ...
    queryData(){
      let that = this
      this.axios.get(`pointDecide/all`).then(res=>{
        if(res.status == 200&&res.data){
          let arr=[...this.bol,...this.checkbox]
          this.isNum=Object.keys(res.data[0]).filter(k=>!arr.includes(k))
    ...
    deleteAgv(index,record){
      this.axios.delete(`pointDecide/${record.pointId}`).then(()=>{
    ...
    handleOk(){
      let that = this
      let str=''
      let requierds=['pointId','area']
      Object.keys(this.record).forEach(k=>{
        console.log(k,this.record[k])
        if((this.record[k]??'')==''&&requierds.includes(k)&&this.record[k]!==0){
          str+='-'+k
        }
      })
      if(str!=''){
        this.$message.info(this.$tcache('admin.pointDecide03', {str}))
        return
      }
      this.$confirm({
    ...
    dealt(value,num){
      if(Array.isArray(value)){
        if(value.length==0){
          value=null;
          if(num==1){this.record.buffers=value;}
        }else{
    ...
    save(){
      ...
      //   if(this.Number)
      // })
      this.dealt(this.record.buffers,1)
      console.log(this.record)
      if(this.record.enabled==null){
        this.record.enabled=false
      }
      this.axios.post(`pointDecide/save`,this.record).then(()=>{
        if(this.type=='edit'){
          this.isSave=false
          this.saveId=this.record.pointId
        }else{
